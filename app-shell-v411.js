...
(function(){
  ...
  function css(){if(document.getElementById('shell411Css'))return;const s=document.createElement('style');s.id='shell411Css';s.textContent=`
    .app-shell.shell-v411 .main{max-width:1180px;margin:0 auto;padding:16px}.shell411-nav{display:flex;gap:7px;flex-wrap:wrap;margin:0 0 14px}.shell411-nav button{min-height:44px;border:1px solid #c7d6e3;border-radius:11px;background:#fff;color:#36536f;padding:0 14px;font-weight:900}.shell411-nav button.on{background:#173b66;border-color:#173b66;color:#fff}.shell411-safety-home{display:grid;gap:13px}.shell411-head{display:flex;align-items:flex-start;justify-content:space-between;gap:10px;flex-wrap:wrap}.shell411-head h2{margin:0;color:#173b66;font-size:25px}.shell411-head p{margin:5px 0 0;color:#6a7e90}.shell411-stat{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px}.shell411-stat>div{padding:14px;border:1px solid #d8e3ed;border-radius:13px;background:#fff}.shell411-stat span{display:block;color:#718294;font-size:11px;font-weight:850}.shell411-stat b{display:block;margin-top:5px;color:#173b66;font-size:25px}.shell411-site-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:9px}.shell411-site{border:1px solid #d8e3ed;border-radius:13px;background:#fff;padding:13px;text-align:left;color:inherit}.shell411-site h3{margin:0;color:#173b66;font-size:15px}.shell411-site div{display:flex;gap:10px;margin-top:10px}.shell411-site span{font-size:10px;color:#748698}.shell411-site b{display:block;margin-top:2px;color:#304c65;font-size:17px}.shell411-site small{display:block;margin-top:8px;color:#8090a0}.shell411-recent{display:grid;gap:7px}.shell411-recent button{display:grid;grid-template-columns:110px 130px minmax(0,1fr) auto;gap:9px;align-items:center;width:100%;padding:11px;border:1px solid #dce5ed;border-radius:11px;background:#fff;text-align:left;color:inherit}.shell411-recent b{color:#173b66}.shell411-recent span,.shell411-recent small{font-size:11px;color:#65798c}.shell411-fatal{max-width:680px;margin:40px auto;padding:24px;border:2px solid #e1b5b5;border-radius:18px;background:#fff7f7}.shell411-fatal h2{color:#8d3333}.shell411-fatal button{min-height:48px;border:0;border-radius:10px;background:#1e5d91;color:#fff;padding:0 16px;font-weight:900}@media(max-width:800px){.shell411-stat,.shell411-site-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.shell411-recent button{grid-template-columns:85px minmax(0,1fr) auto}.shell411-recent .site{display:none}}@media(max-width:560px){.app-shell.shell-v411 .main{padding:10px 8px}.shell411-nav{overflow-x:auto;flex-wrap:nowrap;padding-bottom:3px}.shell411-nav button{white-space:nowrap;min-height:42px}.shell411-head h2{font-size:22px}.shell411-site-grid{gap:7px}.shell411-site{padding:11px}.shell411-recent button{grid-template-columns:78px minmax(0,1fr) auto;padding:9px}.shell411-recent .summary{font-size:11px}}
  ...
  function buildShell(u){
    ...
    document.querySelectorAll('[data-shell-view]').forEach(b=>b.onclick=()=>{currentView=b.dataset.shellView;try{enlPlatformSection=currentView==='home'?'hub':'incident';localStorage.setItem(ENL_PLATFORM_SECTION_KEY,enlPlatformSection)}catch(e){}renderShell(u)});
  ...
  function safetyStats(){const all=[...(data.incidents||[])];return {all,reported:all.filter(i=>i.status==='reported').length,rejected:all.filter(i=>i.status==='rejected').length,approved:all.filter(i=>['approved','closed'].includes(i.status)).length}}
  function renderSafetyHome(root,u){
    const s=safetyStats(),sites=[...(data.sites||[])].filter(x=>x.id!=='site-hq').sort((a,b)=>String(a.name).localeCompare(String(b.name),'ko')),recent=[...s.all].sort((a,b)=>new Date(b.occurredAt||0)-new Date(a.occurredAt||0)).slice(0,8);
    root.innerHTML=`<section class="shell411-safety-home"><section class="panel"><div class="shell411-head"><div><div class="ey">ALL SITES</div><h2>전체 사업장 사고현황</h2><p>사업장별 사고와 검토상태를 한 화면에서 확인합니다.</p></div><button type="button" class="primary" id="safetyAll411">전체 사고 보기</button></div></section><div class="shell411-stat"><div><span>전체 사고</span><b>${s.all.length}</b></div><div><span>검토대기</span><b>${s.reported}</b></div><div><span>반려</span><b>${s.rejected}</b></div><div><span>승인·종결</span><b>${s.approved}</b></div></div><section class="panel"><div class="section-head"><div><h2>사업장별 사고</h2><p>사업장을 누르면 해당 현장 사고만 확인합니다.</p></div></div><div class="shell411-site-grid">${sites.map(site=>{const arr=s.all.filter(i=>String(i.siteId)===String(site.id)),review=arr.filter(i=>i.status==='reported').length,latest=[...arr].sort((a,b)=>new Date(b.occurredAt||0)-new Date(a.occurredAt||0))[0];return `<button type="button" class="shell411-site" data-safety-site="${escx(site.id)}"><h3>${escx(site.name)}</h3><div><span>전체<b>${arr.length}</b></span><span>검토대기<b>${review}</b></span></div><small>${latest?'최근 '+String(latest.occurredAt||'').slice(0,10):'사고기록 없음'}</small></button>`}).join('')}</div></section><section class="panel"><div class="section-head"><div><h2>최근 사고보고</h2><p>최근 접수 순으로 표시합니다.</p></div></div><div class="shell411-recent">${recent.map(i=>`<button type="button" data-safety-inc="${escx(i.id)}"><span>${escx(String(i.occurredAt||'').slice(0,10))}</span><b class="site">${escx(siteName(i.siteId))}</b><span class="summary">${escx(i.summary||i.eventType||'사고보고')}</span><span>${statusBadge(i.status)}</span></button>`).join('')||'<div class="empty compact">등록된 사고가 없습니다.</div>'}</div></section></section>`;
    document.getElementById('safetyAll411').onclick=()=>{currentView='incidents';renderShell(u)};root.querySelectorAll('[data-safety-inc]').forEach(b=>b.onclick=()=>openIncidentModal(b.dataset.safetyInc,true,u));root.querySelectorAll('[data-safety-site]').forEach(b=>b.onclick=()=>renderSafetySite(root,u,b.dataset.safetySite));
  }
  function renderSafetySite(root,u,siteId){const arr=[...(data.incidents||[])].filter(i=>String(i.siteId)===String(siteId)).sort(sortIncidents);root.innerHTML=`<section class="panel"><div class="section-head"><div><div class="ey">SITE RECORDS</div><h2>${escx(siteName(siteId))} 사고목록</h2><p>${arr.length}건</p></div><button type="button" class="secondary" id="safetyHome411">← 전체 사업장</button></div>${typeof incidentTable==='function'?incidentTable(arr,true,u):''}</section>`;document.getElementById('safetyHome411').onclick=()=>renderSafetyHome(root,u);if(typeof bindIncidentRows==='function')bindIncidentRows(true,u)}
  ...
  renderShell=function(u){
    if(!u||u.active===false)return renderLogin();
    css();buildShell(u);const root=document.getElementById('view');route(root,u);return root;
  };
