...
(function(){
  ...
  function navItems(u){if(isField(u))return [];if(u.role==='safety')return [['home','사고현황'],['report','사고 등록'],['incidents','전체 사고'],['actions','사고 조치'],['more','사용자·현장 설정']];return [['home','사고현황'],['incidents','승인 사고']]}
  ...
  async function openSafetyReview(id,u){
    let i=(data.incidents||[]).find(x=>String(x.id)===String(id));
    if(!i&&typeof window.enlIncidentPullNow==='function'){try{await window.enlIncidentPullNow()}catch(e){}i=(data.incidents||[]).find(x=>String(x.id)===String(id))}
    if(!i)return alert('해당 사고자료를 서버에서 찾지 못했습니다. 사고현황을 새로 불러온 뒤 다시 시도해 주세요.');
    if(typeof openIncidentModal!=='function')return alert('사고 검토 화면을 불러오지 못했습니다.');
    openIncidentModal(i.id,true,u);
  }
  ...
  function renderSafetyList(root,u,filter=safetyListFilter){
    ...
    document.getElementById('backSafetyHome411').onclick=()=>{currentView='home';renderShell(u)};
    root.querySelectorAll('[data-inc-id]').forEach(r=>r.onclick=()=>openSafetyReview(r.dataset.incId,u));
  }

  function renderSafetyHome(root,u){
    const s=safetyStats(),sites=[...(data.sites||[])].filter(x=>x.id!=='site-hq').sort((a,b)=>String(a.name).localeCompare(String(b.name),'ko')),recent=[...s.all].sort((a,b)=>new Date(b.occurredAt||0)-new Date(a.occurredAt||0)).slice(0,3),ready=typeof window.enlIncidentServerReady==='function'?window.enlIncidentServerReady():true;
    root.innerHTML=`<section class="shell411-safety-home"><section class="panel"><div class="shell411-head"><div><div class="ey">ALL SITES</div><h2>전체 사업장 사고현황</h2><p>사고 상태나 사업장을 누르면 해당 사고목록으로 바로 이동합니다.</p>${ready?'':'<div class="shell411-sync">서버의 최신 사고자료를 확인하고 있습니다…</div>'}</div><div class="shell411-head-actions"><button type="button" class="secondary" id="safetyAdd411">과거 사고 등록</button><button type="button" class="primary" id="safetyAll411">전체 사고 보기</button></div></div></section><div class="shell411-stat"><button type="button" data-safety-status="all"><span>전체 사고</span><b>${s.all.length}</b></button><button type="button" data-safety-status="reported"><span>검토대기</span><b>${s.reported}</b></button><button type="button" data-safety-status="rejected"><span>반려</span><b>${s.rejected}</b></button><button type="button" data-safety-status="approved"><span>승인</span><b>${s.approved}</b></button><button type="button" data-safety-status="closed"><span>종결</span><b>${s.closed}</b></button></div><section class="panel"><div class="section-head"><div><h2>최근 사고보고</h2><p>최근 접수된 사고 3건입니다.</p></div></div><div class="shell411-recent">${recent.map(i=>`<button type="button" data-safety-inc="${escx(i.id)}"><span>${escx(String(i.occurredAt||'').slice(0,10))}</span><b class="site">${escx(siteName(i.siteId))}</b><span class="summary">${escx(String(i.summary||i.eventType||'사고보고').slice(0,80))}</span><span>${statusBadge(i.status)}</span></button>`).join('')||'<div class="empty compact">등록된 사고가 없습니다.</div>'}</div></section><section class="panel"><div class="section-head"><div><h2>사업장별 사고</h2><p>사업장을 누르면 해당 현장의 사고목록을 확인합니다.</p></div></div><div class="shell411-site-grid">${sites.map(site=>{const arr=s.all.filter(i=>String(i.siteId)===String(site.id)),review=arr.filter(i=>i.status==='reported').length,latest=[...arr].sort((a,b)=>new Date(b.occurredAt||0)-new Date(a.occurredAt||0))[0];return `<button type="button" class="shell411-site" data-safety-site="${escx(site.id)}"><h3>${escx(site.name)}</h3><div><span>전체<b>${arr.length}</b></span><span>검토대기<b>${review}</b></span></div><small>${latest?'최근 '+String(latest.occurredAt||'').slice(0,10):'사고기록 없음'}</small></button>`}).join('')}</div></section></section>`;
    document.getElementById('safetyAdd411')?.addEventListener('click',()=>openSafetyReport(u));
    ...
    root.querySelectorAll('[data-safety-inc]').forEach(b=>b.onclick=()=>openSafetyReview(b.dataset.safetyInc,u));
  ...
  function route(root,u){
    ...
    if(u.role==='safety'){
      ...
      if(currentView==='report')return typeof renderUnifiedReport==='function'?renderUnifiedReport(root,u):root.innerHTML='<div class="empty">사고 입력화면을 불러오지 못했습니다.</div>';
