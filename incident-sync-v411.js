...
(function(){
  ...
  function persistRemote(next){data.incidents=next.sort((x,y)=>new Date(y.occurredAt||0)-new Date(x.occurredAt||0));applyingRemote=true;try{baseSaveData()}finally{applyingRemote=false}lastIds=new Set(data.incidents.map(x=>x.id));try{window.enlAuditSnapshotRefresh?.()}catch(e){}}
  function merge(remote){
    if(!Array.isArray(remote))return false;
    const a=actor();
    if(a?.role==='worker'){
      const next=remote.filter(r=>r?.id&&r.siteId===a.siteId);
      const before=JSON.stringify((data.incidents||[]).map(i=>[i.id,i.updatedAt||'',!!i.workerPublicOnly,i.status,i.workerPublic||null]));
      const after=JSON.stringify(next.map(i=>[i.id,i.updatedAt||'',!!i.workerPublicOnly,i.status,i.workerPublic||null]));
      if(before!==after){persistRemote(next);return true}
      return false;
    }
    const map=new Map((data.incidents||[]).map(i=>[i.id,i]));let changed=false;
    for(const r of remote){if(!r?.id)continue;if(a?.role==='field'&&r.siteId!==a.siteId)continue;const local=map.get(r.id);if(!local){map.set(r.id,r);changed=true;continue}const lt=new Date(local.updatedAt||local.createdAt||0).getTime(),rt=new Date(r.updatedAt||r.createdAt||0).getTime();if(rt>lt){map.set(r.id,r);changed=true}}
    if(changed){let next=[...map.values()];if(a?.role==='field')next=next.filter(i=>i.siteId===a.siteId);persistRemote(next)}return changed
  }
