...
begin
 if p_op='create' then
  ...
  insert into public.ai_workflows(source_type,source_id,task_type,status,priority,current_agent,input_summary,input_payload,requested_by,request_id)
   values(coalesce(p_data->>'source_type','incident'),p_data->>'source_id',case when p_data->>'source_type'='safety_question' then 'safety_review' else 'incident_review' end,'queued',coalesce(p_data->>'priority','normal'),agents[1],p_data->>'summary',p_data->'incident',p_actor,(p_data->>'request_id')::uuid) returning * into w;
