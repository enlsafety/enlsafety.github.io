...
(function(){
  ...
  function allowedIncidents(u){
    ...
    if(['manager','executive'].includes(role))return all.filter(i=>['reported','supplement','supplement_submitted','approved','closed'].includes(String(i?.status||'')));
    return [];
  }
  ...
  function statusText(i){const s=String(i?.status||'');return s==='closed'?'종결':s==='approved'?'최종승인':s==='supplement_submitted'?'보완검토대기':s==='supplement'?'보완대기':s==='rejected'?'반려':s==='reported'?'즉시보고 검토대기':'진행중'}
