/* E&L Accident Report App v3.8.1 - Kakao-safe controller */
(function(){
  const VERSION='3.8.1';
  ...
  function stableUrl(){
    const u=new URL('stable381.html',location.href);
    u.searchParams.set('_ts',String(Date.now()));
    u.searchParams.set('v',VERSION);
    return u.toString();
  }
  function refreshNow(){
    try{location.replace(stableUrl())}catch(e){location.href='stable381.html?v='+encodeURIComponent(VERSION)}
  }
  ...
  function injectHeader(){
    ...
    top.insertBefore(box,user);box.querySelector('button').onclick=refreshNow;
  }
