...
(function(){
  ...
  function workerPublicCards(arr){
    return arr.map(i=>`<div class="field411-public-card"><span class="public-date">${escx(dateOnly(i.occurredAt))}</span><div class="field411-public-main"><b>${escx(publicSummary(i))}</b><span>개인 식별정보·진단정보·금액·사진·상세 경위는 표시하지 않습니다.</span></div><div class="field411-public-state">${statusBadgeHtml(i)}<span class="field411-badge">조치 ${escx(actionText(i.corrective?.status))}</span></div></div>`).join('')||'<div class="field411-empty">현재 표시할 사업장 사고가 없습니다.</div>';
  }
  ...
  function renderRecords(root,u){
    ...
    if(isManager(u)){
      ...
      root.innerHTML=`<section class="field411-screen"><section class="panel"><div class="field411-head"><div><div class="ey">SITE RECORDS</div><h2>사고 기록</h2><p>우리 사업장 사고현황과 내가 작성한 사고보고를 확인합니다.</p></div></div><div class="field411-privacy-note">일반근로자에게는 같은 사업장 사고의 업무상 필요한 요약정보만 표시합니다. 사고자·보고자 이름, 진단·치료정보, 진료비·복구비, 사진, 구체적인 사고경위는 다른 근로자에게 공개하지 않습니다.</div></section><section class="panel field411-section"><div class="field411-section-head"><div><h3>우리 사업장 사고 요약</h3><p>발생일·사고구분·유형·처리상태만 확인할 수 있습니다.</p></div></div><div class="field411-list">${workerPublicCards(all)}</div></section><section class="panel field411-section"><div class="field411-section-head"><div><h3>내가 보고한 사고</h3><p>내 보고는 처리상태와 반려사유를 확인하고 필요 시 수정할 수 있습니다.</p></div></div><div class="field411-list">${myReportCards(mine,u)}</div></section></section>`;
    }
