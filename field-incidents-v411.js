...
(function(){
  ...
  const isOwner=u=>String(u?.position||u?.jobTitle||'')==='현장소장';
  ...
  function renderRecords(root,u){
    css();const arr=visible(u);const scope=u.role==='worker'&&!isManager(u)?'내가 보고한 사고기록':`${siteName(u.siteId)} 사고기록`;
    root.innerHTML=`<section class="field411-screen"><section class="panel"><div class="field411-head"><div><div class="ey">SITE RECORDS</div><h2>사고 기록</h2><p>${escx(scope)}을 확인합니다.</p></div></div></section><div class="field411-list">${arr.map(i=>`<article class="field411-card ${i.status==='rejected'?'rejected':''}" data-field-record="${escx(i.id)}"><div class="field411-top"><div>${statusBadgeHtml(i)}</div><span class="date">${escx(dateText(i.occurredAt))}</span></div><h3>${escx(i.eventType||'사고 보고')}</h3><p class="summary">${escx(i.summary||'-')}</p><div class="field411-meta"><span>보고자 ${escx(i.reporterName||'-')}</span><span>${escx(siteName(i.siteId))}</span><span>조치 ${escx(actionText(i.corrective?.status))}</span></div>${i.status==='rejected'&&(i.rejectionNote||i.safetyNote)?`<div class="field411-reject"><b>반려사유</b><br>${escx(i.rejectionNote||i.safetyNote)}</div>`:''}<div class="field411-actions">${isOwner(u)?`<button type="button" class="primary" data-field-open="${escx(i.id)}">사고보고서 보기</button>`:''}${i.status==='rejected'&&typeof window.enlCanEditIncident==='function'&&window.enlCanEditIncident(i,u)?`<button type="button" data-field-edit="${escx(i.id)}">수정 후 재제출</button>`:''}</div></article>`).join('')||'<div class="field411-empty">표시할 사고기록이 없습니다.<br>사고를 보고하면 이 화면에 처리상태와 기록이 표시됩니다.</div>'}</div></section>`;
    back(root,u);
    ...
    root.querySelectorAll('[data-field-edit]').forEach(b=>b.onclick=()=>{const i=(data.incidents||[]).find(x=>String(x.id)===String(b.dataset.fieldEdit));if(!i)return;try{openEditIncidentModal(i,u)}catch(e){console.error(e);alert('수정 화면을 불러오지 못했습니다.')}});
  }
  window.enlRenderFieldRecords=renderRecords;

  function renderActions(root,u){
    css();const arr=visible(u);const scope=u.role==='worker'&&!isManager(u)?'내 사고보고의 후속조치':`${siteName(u.siteId)} 사고 후속조치`;
