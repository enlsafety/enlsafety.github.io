/* E&L Accident Report App v3.7.8 - stable version / refresh controller */
const ENL_DEPLOY_VERSION='3.7.8';
...
async function enlFetchRemoteVersion(timeoutMs=2500){
  ...
  try{const r=await fetch(`version.json?ts=${Date.now()}&rnd=${Math.random().toString(36).slice(2)}`,{cache:'no-store',headers:{'Cache-Control':'no-cache, no-store, must-revalidate','Pragma':'no-cache'},signal:controller?.signal});if(!r.ok)return '';const j=await r.json();return String(j.version||j.build||'').trim()}catch(e){return ''}finally{if(timer)clearTimeout(timer)}
}
...
function enlReloadNow(version=ENL_DEPLOY_VERSION){const u=new URL(location.href);u.searchParams.set('appv',String(version));u.searchParams.set('_reload',String(Date.now()));u.searchParams.set('_fresh','1');u.searchParams.set('_cb',Math.random().toString(36).slice(2));location.replace(u.toString())}
function enlHardRefresh(){document.querySelectorAll('#appRefreshBtn,#loginRefreshBtn').forEach(b=>{b.disabled=true;b.textContent='새로고침 중…'});enlDeleteBrowserCachesBestEffort();setTimeout(()=>enlReloadNow(ENL_DEPLOY_VERSION),60);setTimeout(()=>{try{location.href=`${location.pathname}?appv=${ENL_DEPLOY_VERSION}&_reload=${Date.now()}&_fresh=1&_cb=${Math.random().toString(36).slice(2)}`}catch(e){}},1600)}
...
function enlInjectHeaderRefresh(){enlRefreshStyle();const top=document.querySelector('.topbar'),user=document.querySelector('.topbar .user-wrap');if(!top||!user)return;document.getElementById('headerUpdateControl')?.remove();const box=document.createElement('div');box.id='headerUpdateControl';box.className='header-update-control';box.innerHTML=`<span class="header-version" id="appVersionLabel">v${ENL_DEPLOY_VERSION}</span><button type="button" class="header-refresh-btn" id="appRefreshBtn">새로고침</button>`;top.insertBefore(box,user);document.getElementById('appRefreshBtn').onclick=enlHardRefresh}
function enlInjectLoginRefresh(){enlRefreshStyle();const card=document.querySelector('.login-card');if(!card)return;document.getElementById('loginRefreshBox')?.remove();const box=document.createElement('div');box.id='loginRefreshBox';box.className='login-update-box';box.innerHTML=`<span>앱 버전 <b id="loginVersionLabel">v${ENL_DEPLOY_VERSION}</b></span><button type="button" class="app-refresh-btn" id="loginRefreshBtn">새로고침</button>`;card.appendChild(box);document.getElementById('loginRefreshBtn').onclick=enlHardRefresh}
async function enlCheckRemoteVersion(){const v=await enlFetchRemoteVersion();if(!v)return;document.querySelectorAll('#appVersionLabel,#loginVersionLabel').forEach(x=>x.textContent=enlIsNewer(v)?`v${ENL_DEPLOY_VERSION} → v${v}`:`v${ENL_DEPLOY_VERSION}`);if(!enlIsNewer(v))return;let target='',at=0;try{target=localStorage.getItem(ENL_AUTO_REFRESH_TARGET_KEY)||'';at=Number(localStorage.getItem(ENL_AUTO_REFRESH_AT_KEY)||0)}catch(e){}if(target===v&&Date.now()-at<90000)return;try{localStorage.setItem(ENL_AUTO_REFRESH_TARGET_KEY,v);localStorage.setItem(ENL_AUTO_REFRESH_AT_KEY,String(Date.now()))}catch(e){}enlDeleteBrowserCachesBestEffort();enlReloadNow(v)}
