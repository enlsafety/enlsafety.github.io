...
(function(){
  'use strict';
  const VERSION='4.1.1-r20-duplicate-site1';
  ...
  function renderOptions(options){
    ...
    wrap.hidden=false;wrap.innerHTML=`<details class="login411-aff${duplicateSiteName?' login411-aff-duplicate':''}" open><summary>소속 선택 <span>⌄</span></summary>${duplicateSiteName?'<div class="login411-duplicate-warning" role="alert">동명이인이 있습니다. 본인 사업장을 꼭 확인해 선택해 주세요.</div>':''}<div class="login411-list">${lookupOptions.map((o,i)=>`<button type="button" class="login411-aff-btn" data-login411-aff="${i}"><span><b>${ex(o.affiliationLabel||'-')}</b><small>${ex(o.position||o.roleLabel||'')}${o.kind==='hq'?` · ${ex(o.roleLabel||roleName(o.role))}`:o.requiresPassword?' · 비밀번호 필요':''}</small></span><span>›</span></button>`).join('')}</div></details>`;
