...
(function(){
  ...
  function renderOptions(options){
    ...
    const siteIds=[...new Set(lookupOptions.filter(o=>o?.kind==='site'&&o?.siteId).map(o=>String(o.siteId)))];
    const duplicateSiteName=siteIds.length>1;
    wrap.hidden=false;wrap.innerHTML=`<details class="login411-aff${duplicateSiteName?' login411-aff-duplicate':''}" open><summary><span class="login411-summary-label">소속 선택</span><span class="login411-summary-arrow">⌄</span></summary>${duplicateSiteName?'<div class="login411-duplicate-warning" role="alert">동명이인이 있습니다. 본인 사업장을 꼭 확인해 선택해 주세요.</div>':''}<div class="login411-list">${lookupOptions.map((o,i)=>`<button type="button" class="login411-aff-btn" data-login411-aff="${i}"><span><b>${ex(o.affiliationLabel||'-')}</b><small>${ex(o.position||o.roleLabel||'')}${o.kind==='hq'?` · ${ex(o.roleLabel||roleName(o.role))}`:o.requiresPassword?' · 비밀번호 필요':''}</small></span><span>›</span></button>`).join('')}</div></details>`;
  ...
  function mergeHq(user,passwordHash=''){
    ...
    Object.assign(x,{username:user.name,name:user.name,role,department:user.department||'',position:user.position||'',email:user.email||'',pushEnabled:user.pushEnabled!==false,emailEnabled:user.emailEnabled!==false,siteId:null,active:user.active!==false,updatedAt:user.updatedAt||nowISO()});if(passwordHash)x.passwordHash=passwordHash;saveData();return x;
  }
  ...
  openUserModal=function(user,u){const isNew=!user,role=roleNorm(user?.role)||'manager';openModal(`<div class="modal-head"><div><h2>${isNew?'본사 사용자 생성':'본사 사용자 수정'}</h2></div><button class="x" data-close>×</button></div><form id="hqUserForm411"><div class="formgrid"><label class="lbl"><span>이름 *</span><input id="hqName411" value="${ex(user?.name||'')}" required></label><label class="lbl"><span>소속사업부</span><input id="hqDept411" value="${ex(user?.department||'')}"></label><label class="lbl"><span>직급</span><input id="hqPos411" value="${ex(user?.position||'')}"></label><label class="lbl"><span>역할군</span><select id="hqRole411"><option value="safety" ${role==='safety'?'selected':''}>안전관리자</option><option value="manager" ${role==='manager'?'selected':''}>관리자</option><option value="executive" ${role==='executive'?'selected':''}>경영진</option></select></label><label class="lbl"><span>이메일</span><input id="hqEmail411" type="email" value="${ex(user?.email||'')}" placeholder="name@company.com"></label><label class="lbl"><span>앱 푸시 알림</span><select id="hqPush411"><option value="1" ${user?.pushEnabled!==false?'selected':''}>ON</option><option value="0" ${user?.pushEnabled===false?'selected':''}>OFF</option></select></label><label class="lbl"><span>이메일 알림</span><select id="hqMail411"><option value="1" ${user?.emailEnabled!==false?'selected':''}>ON</option><option value="0" ${user?.emailEnabled===false?'selected':''}>OFF</option></select></label></div><div class="help" style="margin:10px 0">앱 푸시와 이메일 알림은 기본 ON입니다. 관리자·경영진의 긴급사고 알림은 OFF 설정과 관계없이 강제 수신됩니다.</div><label class="lbl"><span>${isNew?'초기 비밀번호 *':'새 비밀번호 (선택)'}</span><input id="hqPw411" type="password" ${isNew?'required':''}></label><button class="primary full">저장</button></form>`);document.getElementById('hqUserForm411').onsubmit=async ev=>{ev.preventDefault();const name=document.getElementById('hqName411').value.trim(),department=document.getElementById('hqDept411').value.trim(),position=document.getElementById('hqPos411').value.trim(),role=document.getElementById('hqRole411').value,email=document.getElementById('hqEmail411').value.trim(),pushEnabled=document.getElementById('hqPush411').value!=='0',emailEnabled=document.getElementById('hqMail411').value!=='0',pw=document.getElementById('hqPw411').value,hash=pw?await sha256(pw):(user?.passwordHash||'');if(!hash)return alert('비밀번호를 입력해 주세요.');try{const r=await api({action:'hq_upsert',actor:actor(u),user:{id:user?.id||uid('u'),name,department,position,role,email,pushEnabled,emailEnabled,active:true,passwordHash:hash}});mergeHq({...r.user,id:r.user?.id||user?.id},hash);await syncHqUsers(currentUser()||u);closeModal();renderShell(currentUser()||u)}catch(e){alert(e?.message==='invalid_email'?'이메일 형식을 확인해 주세요.':'본사 사용자 계정을 저장하지 못했습니다.')}}}
