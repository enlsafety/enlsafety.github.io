import { diagnostic, friendlyError, requestWithBackoff } from "./service-errors.ts";
...
function openAIHeaders(){const headers:Record<string,string>={Authorization:'Bearer '+(Deno.env.get('OPENAI_API_KEY')||''),'Content-Type':'application/json'};
if(Deno.env.get('OPENAI_PROJECT_ID'))headers['OpenAI-Project']=Deno.env.get('OPENAI_PROJECT_ID')!;
if(Deno.env.get('OPENAI_ORG_ID'))headers['OpenAI-Organization']=Deno.env.get('OPENAI_ORG_ID')!;
return headers;}
async function callAgent(opts:{name:string,model:string,instructions:string,input:any,web?:boolean,effort?:string,onRetry?:(event:any)=>Promise<void>}){const key=Deno.env.get("OPENAI_API_KEY")||"";if(!key)throw new Error("openai_not_configured");const body:any={store:false,model:opts.model,instructions:opts.instructions,input:JSON.stringify(opts.input),reasoning:{effort:opts.effort||"low"},max_output_tokens:7000,text:{format:{type:"json_schema",name:"enl_ai_safety_result",strict:true,schema:RESULT_SCHEMA}}};if(opts.web){body.tools=[{type:"web_search",search_context_size:"medium",filters:{allowed_domains:OFFICIAL_DOMAINS}}];body.include=["web_search_call.action.sources"];body.tool_choice="required";}const headers=openAIHeaders();
const {raw,attempts,requestId,duration_ms}=await requestWithBackoff(body,{headers,onRetry:opts.onRetry});
const text=extractText(raw);if(!text)throw new Error("openai_empty_output");let parsed:any;try{parsed=JSON.parse(text)}catch{throw new Error("openai_invalid_json")};const result=normalizeResult(parsed);if(opts.web){
 ...
 result.official_sources=result.official_sources.filter((x:{url:string})=>verified.has(x.url));
 for(const [url,source] of verified)if(!result.official_sources.some((x:{url:string})=>x.url===url)&&result.official_sources.length<12)result.official_sources.push({url,title:clean(source.title)||url,source_type:url.includes("kosha.or.kr")?"안전보건공단 자료 · 법령 아님":"공식 검색 근거"});
 }return {result,attempts,requestId,duration_ms,responseId:clean(raw?.id),model:clean(raw?.model)||opts.model,usage:raw?.usage?{input_tokens:raw.usage.input_tokens,output_tokens:raw.usage.output_tokens,total_tokens:raw.usage.total_tokens}:null}}
...
const PUBLIC_WORKFLOW='id,source_type,source_id,task_type,status,priority,current_agent,input_summary,result_summary,requires_human_approval,requested_by,approved_by,approved_at,created_at,started_at,completed_at,updated_at,step_index,retry_of,retry_not_before';
...
async function kick(db:any,id:string){
 try{
 ...
 }catch{await transition(db,'dispatch_fail',id,{error:'업무 인계 연결에 실패했습니다. 재시도해 주세요.'});}
}
function background(p:Promise<any>){EdgeRuntime.waitUntil(p.catch(()=>{/* The database watchdog expires orphaned jobs; polling never launches work. */}));}
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(action==='inspect_ai_configuration'){
   const response=await fetch('https://api.openai.com/v1/models',{headers:openAIHeaders(),signal:AbortSignal.timeout(15000)});
   const raw=await response.json();const wanted=[DEFAULT_MODEL,LEGAL_MODEL,DIRECTOR_MODEL,'gpt-5.4'];
   return json(req,{ok:true,configuration:health(),projectHeaderConfigured:!!Deno.env.get('OPENAI_PROJECT_ID'),organizationHeaderConfigured:!!Deno.env.get('OPENAI_ORG_ID'),models:response.ok?(raw.data||[]).filter((m:any)=>wanted.includes(m.id)).map((m:any)=>m.id):[],diagnostic:response.ok?null:diagnostic(response,raw,[Deno.env.get('OPENAI_API_KEY')||''])});
  }
  ...
  if(action==='review_incident'||action==='review_question'){
   if(!Deno.env.get('OPENAI_API_KEY'))return json(req,{ok:false,message:'openai_not_configured'},503);
   if((action==='review_incident'&&!clean(body.incidentId))||(action==='review_question'&&(clean(body.question).length<10||clean(body.question).length>4000))||!uuid(body.requestId))return json(req,{ok:false,message:'invalid_request'},400);
   const incident=action==='review_question'?minimized({summary:body.question,siteName:'안전관리 질의',category:'question'}):await sourceIncident(db,clean(body.incidentId));
   const w=await transition(db,'create',null,{source_type:action==='review_question'?'safety_question':'incident',source_id:action==='review_question'?'question':clean(body.incidentId),request_id:body.requestId,incident,summary:(incident.site_name||incident.site_id)+' · '+incident.summary.slice(0,600),priority:incident.potential_major?'urgent':'normal'},actor.id);
   background(kick(db,w.id));return json(req,{ok:true,workflowId:w.id,status:w.status},202);
  }
  if(action==='decide_workflow'||action==='retry_workflow'){
   const prev=checked(await db.from('ai_workflows').select('source_id,source_type,input_payload').eq('id',body.workflowId).maybeSingle());if(!prev)return json(req,{ok:false,message:'not_found'},404);
   const incident=(action==='retry_workflow'||body.decision==='re_review')?(prev.source_type==='safety_question'?prev.input_payload:await sourceIncident(db,prev.source_id)):undefined;if(incident&&body.decision==='re_review')incident.review_focus=redactText(body.note).slice(0,2000);
   ...
   if(w.status==='queued')background(kick(db,w.id));return json(req,{ok:true,workflowId:w.id,status:w.status});
  }
