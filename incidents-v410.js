...
(function(){
  ...
  function css(){
    ...
    const s=document.createElement('style');s.id='incident410Css';s.textContent=`
      .p-rejected{background:#fff0f0!important;color:#a02f2f!important;border:1px solid #efb7b7!important}
      .incident-private-list{display:grid;gap:10px}.incident-private-card{border:2px solid #d7e2ec;border-radius:14px;background:#fff;padding:14px}.incident-private-card.rejected{border-color:#e2abab;background:#fffafa}
      .incident-private-top{display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap}.incident-private-card h3{margin:9px 0 5px;color:#173b66;font-size:17px}.incident-private-meta{display:flex;gap:8px;flex-wrap:wrap;color:#66798b;font-size:12px;font-weight:750}
      .incident-private-note,.reject-note{margin-top:9px;padding:10px 11px;border:1px solid #e9bcbc;border-radius:10px;background:#fff1f1;color:#893737;font-size:13px;line-height:1.5}.incident-private-actions{display:flex;gap:7px;margin-top:10px;flex-wrap:wrap}.incident-private-actions button{min-height:42px;border:0;border-radius:9px;background:#1e5d91;color:#fff;padding:0 14px;font-weight:900}
      .incident-private-lock{margin-top:9px;color:#788a99;font-size:12px}.btn-reject{background:#b13c3c!important;color:#fff!important}.edit-resubmit-note{margin:0 0 12px;padding:11px;border:1px solid #e7b4b4;border-radius:10px;background:#fff3f3;color:#7f3535;font-weight:800;line-height:1.5}
      @media(max-width:620px){.incident-private-card{padding:12px}.incident-private-actions button{width:100%}}
  ...
  function incidentTableFn(arr,admin,u){return originalTable(arr,u||currentUser?.(),admin)}
  function bindIncidentRowsFn(admin,u){
    const viewer=u||currentUser?.();
    document.querySelectorAll('[data-inc-id]').forEach(r=>{const i=(data.incidents||[]).find(x=>String(x.id)===String(r.dataset.incId));r.onclick=i&&canViewOriginal(i,viewer)?()=>openIncidentModalFn(i.id,admin,viewer):null;if(!r.onclick)r.style.cursor='default'});
  }
  ...
  function privateCards(arr,u){
    ...
    return `<div class="incident-private-list">${arr.map(i=>{const editable=canEditIncident(i,u),rejected=i.status==='rejected';return `<article class="incident-private-card ${rejected?'rejected':''}"><div class="incident-private-top"><div>${typeof categoryBadge==='function'?categoryBadge(i.category):''}${statusBadgeHtml(i.status)}</div><b>${fmt(i.occurredAt)}</b></div><h3>${esc(i.eventType||'사고 보고')}</h3><div class="incident-private-meta"><span>보고자 ${esc(i.reporterName||'-')}</span><span>${esc(siteName(i.siteId))}</span></div>${rejected&&i.rejectionNote?`<div class="incident-private-note"><b>반려사유</b><br>${esc(i.rejectionNote)}</div>`:''}<div class="incident-private-lock">사고보고서 원본은 현장소장·파트장·서무가 조회할 수 있습니다.</div>${editable?`<div class="incident-private-actions"><button type="button" data-rejected-edit="${esc(i.id)}">${i.status==='reported'?'보고 회수 후 수정':'반려 내용 수정 후 재제출'}</button></div>`:''}</article>`}).join('')}</div>`;
  }
  ...
  function renderUnifiedIncidentsFn(root,u){
    ...
    if(isSiteUser(viewer)){
      ...
      if(isManager(viewer)){root.innerHTML=`<div class="panel"><div class="section-head"><div><div class="ey">SITE RECORDS</div><h2>사고 기록</h2><p>${esc(siteName(viewer.siteId))}의 전체 사고보고서를 조회합니다.</p></div></div>${originalTable(arr,viewer,false)}</div>`;bindIncidentRowsFn(false,viewer);return}
    ...
    if(viewer.role==='final'){
      const arr=[...(data.incidents||[])].filter(i=>['approved','closed'].includes(i.status)).sort(sortIncidentList);root.innerHTML=`<div class="panel"><div class="section-head"><div><h2>승인 사고 조회</h2><p>안전관리자가 승인한 사고만 표시됩니다.</p></div></div>${originalTable(arr,viewer,false)}</div>`;bindIncidentRowsFn(false,viewer);return;
    }
    ...
    const refresh=()=>{let arr=[...base];const sf=document.getElementById('siteFilter')?.value||'',st=document.getElementById('statusFilter')?.value||'',cf=document.getElementById('categoryFilter')?.value||'';if(sf)arr=arr.filter(i=>String(i.siteId)===String(sf));if(st)arr=arr.filter(i=>i.status===st;if(false){});if(cf)arr=arr.filter(i=>i.category===cf);const box=document.getElementById('unifiedIncidentTable');if(box)box.innerHTML=originalTable(arr,viewer,true);bindIncidentRowsFn(true,viewer)};
  ...
  function openIncidentModalFn(id,admin,u){
    ...
    if(!i){alert('해당 사고자료를 찾지 못했습니다. 최신 사고현황을 다시 불러와 주세요.');return}
    if(!canViewOriginal(i,viewer)){alert('사고보고서 원본 조회 권한이 없습니다.');return}
    if(typeof openModal!=='function'){alert('사고 검토 화면을 불러오지 못했습니다.');return}
  ...
  function openEditIncidentModalFn(i,u){
    ...
    if(!canEditIncident(i,viewer)){alert('수정 권한이 없습니다.');return}
