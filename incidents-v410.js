/* E&L Accident Report App v4.1.0 - authoritative incident access/review */
(function(){
  'use strict';
  const VERSION='4.1.0';
  ...
  const isSiteOwner=u=>isSiteUser(u)&&position(u)==='현장소장';
  ...
  function canEditIncident(i,u){
    ...
    if(i.status!=='rejected'||!sameSite(i,u))return false;
  ...
  function canViewOriginal(i,u){
    ...
    return sameSite(i,u)&&isSiteOwner(u);
  }
  ...
  function privateCards(arr,u){
    if(!arr.length)return '<div class="empty">표시할 사고가 없습니다.</div>';
    return `<div class="incident-private-list">${arr.map(i=>{const editable=canEditIncident(i,u);const rejected=i.status==='rejected';return `<article class="incident-private-card ${rejected?'rejected':''}"><div class="incident-private-top"><div>${categoryBadge(i.category)}${statusBadge(i.status)}</div><b>${fmt(i.occurredAt)}</b></div><h3>${esc(i.eventType||'사고 보고')}</h3><div class="incident-private-meta"><span>보고자 ${esc(i.reporterName||'-')}</span><span>${esc(siteName(i.siteId))}</span></div>${rejected&&i.rejectionNote?`<div class="incident-private-note"><b>반려사유</b><br>${esc(i.rejectionNote)}</div>`:''}<div class="incident-private-lock">사고보고서 원본은 현장소장만 조회할 수 있습니다.</div>${editable?`<div class="incident-private-actions"><button type="button" data-rejected-edit="${esc(i.id)}">반려 내용 수정 후 재제출</button></div>`:''}</article>`}).join('')}</div>`;
  }
  ...
  renderUnifiedIncidents=function(root,u){
    ...
    if(isSiteUser(actual)||u?.role==='field'){
      const viewer=actual||u;const arr=visibleFieldRecords(viewer);
      if(isSiteOwner(viewer)){
        root.innerHTML=`<div class="panel"><div class="section-head"><div><div class="ey">SITE RECORDS</div><h2>사고 기록</h2><p>${esc(siteName(viewer.siteId))}의 사고보고서 원본을 조회합니다.</p></div></div>${originalTable(arr,viewer,false)}</div>`;bindIncidentRows(false,viewer);return;
      }
      const label=isManager(viewer)?'현장 사고 기록':'내 사고 보고 기록';
      root.innerHTML=`<div class="panel"><div class="section-head"><div><div class="ey">SITE RECORDS</div><h2>${label}</h2><p>${isManager(viewer)?'현장 기록의 상태를 확인하고, 반려된 보고서를 수정할 수 있습니다.':'내가 작성한 보고의 처리상태를 확인합니다.'}</p></div></div>${privateCards(arr,viewer)}</div>`;bindRejectedEdits(root,viewer);return;
    }
    if(u?.role==='final'){
  ...
  openIncidentModal=function(id,admin,u){
    const viewer=u||currentUser?.();const i=(data.incidents||[]).find(x=>String(x.id)===String(id));if(!i)return;
    if(!canViewOriginal(i,viewer)){alert('현장에서는 사고보고서 원본을 현장소장만 조회할 수 있습니다.');return}
