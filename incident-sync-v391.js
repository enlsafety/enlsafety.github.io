/* E&L Incident shared sync v3.9.1 - workers can read/edit site incidents, no delete */
(function(){
  ...
  async function call(body,timeout=9000){const controller=typeof AbortController!=='undefined'?new AbortController():null;const timer=controller?setTimeout(()=>controller.abort(),timeout):null;try{const r=await fetch(API,{method:'POST',headers:{'Content-Type':'application/json','X-ENL-App':CLIENT},body:JSON.stringify(body),signal:controller?.signal});const j=await r.json().catch(()=>({}));if(!r.ok||j?.ok===false)throw new Error(j?.message||`sync_http_${r.status}`);return j}finally{if(timer)clearTimeout(timer)}}
  function merge(remote){if(!Array.isArray(remote))return false;const a=actor(),map=new Map((data.incidents||[]).map(i=>[i.id,i]));let changed=false;for(const r of remote){if(!r?.id)continue;if((a?.role==='field'||a?.role==='worker')&&r.siteId!==a.siteId)continue;const local=map.get(r.id);if(!local){map.set(r.id,r);changed=true;continue}const lt=new Date(local.updatedAt||local.createdAt||0).getTime(),rt=new Date(r.updatedAt||r.createdAt||0).getTime();if(rt>lt){map.set(r.id,r);changed=true}}if(changed){let next=[...map.values()];if(a?.role==='field'||a?.role==='worker')next=next.filter(i=>i.siteId===a.siteId);data.incidents=next.sort((x,y)=>new Date(y.occurredAt||0)-new Date(x.occurredAt||0));applyingRemote=true;try{baseSaveData()}finally{applyingRemote=false}lastIds=new Set(data.incidents.map(x=>x.id));try{window.enlAuditSnapshotRefresh?.()}catch(e){}}return changed}
  async function push(deletedIds=[]){const a=actor();if(!a)return;let incidents=[...(data.incidents||[])];if(a.role==='field'||a.role==='worker')incidents=incidents.filter(i=>i.siteId===a.siteId);const deletions=a.role==='safety'?deletedIds:[];await call({action:'push',actor:a,role:a.role,siteId:a.siteId,incidents,deletedIds:deletions})}
  async function pull(renderIfChanged=true){const a=actor();if(!a)return false;const res=await call({action:'pull',actor:a,role:a.role,siteId:a.siteId});const changed=merge(res?.incidents||[]);if(changed&&renderIfChanged&&!document.querySelector('.modal')&&!['INPUT','TEXTAREA','SELECT'].includes(document.activeElement?.tagName||'')){setTimeout(()=>{try{renderShell(currentUser())}catch(e){}},0)}return changed}
