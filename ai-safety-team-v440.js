...
(function(){
  ...
  function diagnosticReport(runs,events){
   const entries=[...events.filter(e=>e.metadata?.diagnostic).map(e=>e.metadata.diagnostic),...runs.map(r=>r.output_payload?._meta?.error).filter(Boolean)];
   const text=v=>v==null?null:String(v).replace(/\bsk-[A-Za-z0-9_-]+/g,'[key removed]').replace(/Bearer\s+\S+/gi,'Bearer [removed]').slice(0,700);
   const keys=['x-request-id','retry-after','retry-after-ms','x-ratelimit-limit-requests','x-ratelimit-limit-tokens','x-ratelimit-remaining-requests','x-ratelimit-remaining-tokens','x-ratelimit-reset-requests','x-ratelimit-reset-tokens'];
   return entries.map(d=>({http_status:d.http_status??null,error:{type:text(d.type),code:text(d.code),message:text(d.message),param:text(d.param)},headers:Object.fromEntries(keys.map(k=>[k,text(d.headers?.[k])])),model:text(d.model),agent:text(d.agent),job_id:text(d.job_id),retry_count:d.retry_count??null,observed_at:text(d.observed_at||d.ended_at),duration_ms:d.duration_ms??null}));
  }
