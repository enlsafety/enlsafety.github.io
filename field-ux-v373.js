/* E&L Safety v3.7.3 - field UX refinements */
(function(){
  const MIN_PHOTOS=3;
  ...
  function patchFieldHome(){
    ...
    wanted.forEach(key=>{
      const btn=home.querySelector(`[data-field-task="${key}"]`);
      if(!btn)return;
      ...
      if(n)n.textContent=no;if(strong)strong.textContent=title;if(small)small.textContent=desc;
  ...
  function patchInquirySafetyInfo(){
    ...
    box.className='field-safety-contact';
  ...
  function incidentCount(){try{return Array.isArray(incidentPhotos)?incidentPhotos.length:0}catch(e){return 0}}
  function actionCount(){try{return Array.isArray(actionPhotos)?actionPhotos.length:0}catch(e){return 0}}

  function ensurePhotoGuide(kind){
    const form=kind==='incident'?document.getElementById('unifiedReportForm'):(document.getElementById('unifiedCorrectiveForm')||document.getElementById('correctiveForm'));
    if(!form)return;
    const box=form.querySelector('.photo-box');
    if(!box)return;
    box.classList.add('photo-min-three');
    const title=box.querySelector('.photo-head b');
    if(title){
      const base=kind==='incident'?'현장사진':'조치사진';
    ...
    const count=kind==='incident'?incidentCount():actionCount();
    guide.textContent=`필수 · 사진을 최소 ${MIN_PHOTOS}장 이상 등록해 주세요. (현재 ${count}장)`;
  ...
  function photoError(kind){
    ...
    const box=form?.querySelector('.photo-box');
    if(box){
      box.classList.add('photo-min-error');
      box.scrollIntoView({behavior:'smooth',block:'center'});
      setTimeout(()=>box.classList.remove('photo-min-error'),1800);
    }
    alert(`사진은 최소 ${MIN_PHOTOS}장 이상 등록해야 합니다.`);
  }

  function patchActionModal(){
    const form=document.getElementById('unifiedCorrectiveForm')||document.getElementById('correctiveForm');
    if(!form)return;
    const modal=form.closest('.modal');
    if(modal)modal.classList.add('action-modal-lock');
  ...
  function patchAll(){
    patchFieldHome();
    patchInquirySafetyInfo();
    ensurePhotoGuide('incident');
    patchActionModal();
  }
  ...
  if(typeof openUnifiedCorrectiveModal==='function'){
    const baseOpenUnifiedCorrectiveModal=openUnifiedCorrectiveModal;
    openUnifiedCorrectiveModal=function(id,u){
      const result=baseOpenUnifiedCorrectiveModal(id,u);
      setTimeout(patchActionModal,0);
      return result;
    };
  }
  if(typeof openCorrectiveModal==='function'){
    const baseOpenCorrectiveModal=openCorrectiveModal;
    openCorrectiveModal=function(id,u){
      const result=baseOpenCorrectiveModal(id,u);
      setTimeout(patchActionModal,0);
      return result;
    };
  }
  ...
  if(typeof renderPhotoThumbs==='function'){
    const baseRenderPhotoThumbs=renderPhotoThumbs;
    renderPhotoThumbs=function(kind){
      const r=baseRenderPhotoThumbs(kind);
      setTimeout(()=>ensurePhotoGuide(kind==='action'?'action':'incident'),0);
      return r;
    };
  }
